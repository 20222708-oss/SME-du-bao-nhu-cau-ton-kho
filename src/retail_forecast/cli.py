...
from .pipeline import make_forecast, train_pipeline
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "train":
        ...
        if result.forecast is not None:
            result.forecast.to_csv(output_dir / "forecast.csv", index=False)
        print(result.metrics)
        return 0

    if args.command == "forecast":
        forecast = make_forecast(args.data, profile=args.dataset, horizon=args.horizon)
        forecast.to_csv(args.output, index=False)
        print(f"Saved forecast to {args.output}")
        return 0

    if args.command == "export":
        result = train_pipeline(args.data, profile=args.dataset, horizon=30, output_dir=args.output_dir)
